...
namespace Modio.Models
{
    ...
    public class File
    {
        ...
        public string ApprovedPlatforms {
            get
            {
                return string.Join(", ", Platforms.Where(p => p.Status == 1).Select(p => p.PlatformName).ToList());
            }
        }

        public string PendingPlatforms
        {
            get
            {
                return string.Join(", ", Platforms.Where(p => p.Status == 0).Select(p => p.PlatformName).ToList());
            }
        }
        ...
        public DateTime DateAddedLocal
        {
            get
            {
                return DateTimeOffset.FromUnixTimeSeconds(DateAdded).LocalDateTime;
            }
        }
